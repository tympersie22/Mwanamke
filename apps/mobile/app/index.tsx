...
export default function MobileApp() {
  ...
  const logout = async () => { await clearSession(); setSession(null); setActor(null); setRows([]); setMetrics({}); setStep("privacy"); };
  ...
  useEffect(() => {
    if(!session) return;
    let active=true;
    void request("me",session).then(result => { if(!active)return; setActor(result.data);void load(result.data.role === "provider" ? "appointments" : result.data.role === "navigator" ? "navigator/assignments" : result.data.role.endsWith("admin") ? "admin/aggregate" : "providers",session); }).catch(reason => { if(active)setError(reason.message); });
    return () => { active=false; };
  ...
  const chooseLanguage = async (value:"sw"|"en") => {setLanguage(value);try{await AsyncStorage.setItem("mwanamke.language",value);}catch{setError(t("Lugha haijahifadhiwa.","Language preference could not be saved."));}};
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Text style={styles.brand}>MWANAMKE</Text>
    ...
    {step === "language" ? <><Text accessibilityRole="header" style={styles.title}>{t("Karibu. Chagua lugha yako.","Welcome. Choose your language.")}</Text><Text style={styles.body}>{t("Huduma, kwa hatua yako.","Care, at your pace.")}</Text>{button("Kiswahili",()=>void chooseLanguage("sw"),language === "sw")}{button("English",()=>void chooseLanguage("en"),language === "en")}{button(t("Endelea","Continue"),()=>setStep("privacy"),true)}</> : step === "privacy" ? <><Text accessibilityRole="header" style={styles.title}>{t("Nafasi yako ya faragha","Your private care space")}</Text><View style={styles.card}><Text style={styles.body}>{t("Huduma hii si ya dharura. Usitumie kwa usaidizi wa haraka. Taarifa za akaunti na miadi zinahifadhiwa na huduma. Hakuna taarifa za afya zinazohitajika ili kuingia.","This service is not for emergencies. Do not rely on it for urgent help. Account and appointment information is stored by the service. No health details are needed to sign in.")}</Text></View><Text style={styles.body}>{t("Watoa huduma na wafanyakazi hutumia akaunti zilizoalikwa na kuthibitishwa.","Care professionals and staff use invited and verified accounts.")}</Text>{Platform.OS === "web" ? button(t("Endelea kwenye tovuti salama","Continue in secure web portal"),()=>void continueInWebPortal(),true) : button(t("Ingia au jisajili","Sign in or register"),()=>void login(),true)}{button(t("Badili lugha","Change language"),()=>setStep("language"))}</> : <>
      <Text style={styles.verified}>{roleExperience.label}</Text>
      <Text accessibilityRole="header" style={styles.title}>{roleExperience.title}</Text>
      {actor && <View style={styles.nav}>{(actor.role === "patient" ? [["providers",t("Huduma","Care")],["appointments",t("Miadi","Appointments")],["payments",t("Malipo","Payments")]] : actor.role === "provider" ? [["appointments",t("Ratiba","Schedule")],["providers",t("Huduma","Directory")]] : actor.role === "navigator" ? [["navigator/assignments",t("Kazi","Assignments")],["providers",t("Huduma","Directory")]] : [["admin/aggregate",t("Muhtasari","Overview")]]).concat([["privacy/requests",t("Akaunti","Account")]]).map(([endpoint,label])=><View key={endpoint}>{button(label!,()=>{setProvider(null);setService(null);setNotice("");void load(endpoint!);},path===endpoint)}</View>)}</View>}

    ...
    <div className="live-shell" lang={language}>
      ...
      <header className="live-header">
        ...
        <div className="live-header-actions">
          <label className="live-language"><Languages aria-hidden="true" /><span>{t("Lugha", "Language")}</span><select aria-label={t("Chagua lugha", "Choose language")} value={language} onChange={(event) => { const value = event.target.value as Language; setLanguage(value); document.cookie = `mwanamke-language=${value}; Path=/; SameSite=Lax; Max-Age=31536000`; document.documentElement.lang = value; }}><option value="sw">Kiswahili</option><option value="en">English</option></select></label>
            ...
            <div className="live-workspace">
              ...
              <section className="live-data-section" aria-busy={busy}>
                ...
                {path === "privacy/requests" ? <div className="live-account-grid">
                  <section className="live-card"><p className="live-card-eyebrow">{t("Mapendeleo", "Preference")}</p><h3>{t("Lugha ya akaunti", "Account language")}</h3><dl className="live-definition"><div><dt>{t("Iliyohifadhiwa", "Saved")}</dt><dd>{savedLanguage === "sw" ? "Kiswahili" : savedLanguage === "en" ? "English" : "—"}</dd></div><div><dt>{t("Iliyochaguliwa", "Selected")}</dt><dd>{language === "sw" ? "Kiswahili" : "English"}</dd></div></dl><Button variant="primary" disabled={busy || savedLanguage === language} onClick={() => void mutate("profile", { preferredLanguage: language }, "PATCH")}>{savedLanguage === language ? t("Lugha imehifadhiwa", "Language is saved") : t("Hifadhi lugha hii", "Save this language")}</Button></section>
                ...
                {!busy || rows.length > 0 ? <div className={path === "providers" || isServices || isAvailability ? "live-grid" : "live-record-list"}>{displayRows.map((row) => {
                  if (path === "providers") return <article className="live-card" key={row.id}><div className="live-card-topline"><span className="live-verified"><ShieldCheck aria-hidden="true" />{t("Amethibitishwa", "Verified")}</span></div><h3>{row.displayName}</h3><p className="live-card-lead">{language === "sw" ? row.titleSw : row.titleEn}</p><p className="live-meta"><Languages aria-hidden="true" /> {row.languages?.join(", ") || t("Lugha haijatajwa", "Languages not listed")}</p><Button className="live-card-action" onClick={() => navigate(`providers/${row.id}/services`, row, null)}>{actor?.role === "patient" ? t("Ona huduma na bei", "View services and prices") : t("Ona huduma", "View services")}<ArrowRight aria-hidden="true" /></Button></article>;
